...
public class AutoShoot extends SequentialCommandGroup {
    ...
    public AutoShoot() {
        ...
        addCommands(
                ...
                new RunCommand(() -> RobotContainer.indexer.set(1))
                        .beforeStarting(
                                new WaitUntilCommand(RobotContainer.shooter::readyToShoot)
                                        .withTimeout(Parameters.shooter.SHOT_TIME)),

                // finally, do a parallel command group that tells everything to stop and to run the
                // intake again, lets go score again
                new ParallelCommandGroup(
                        new InstantCommand(RobotContainer.shooter::stop),
                        new InstantCommand(RobotContainer.indexer::stop),
                        new InstantCommand(
                                () -> RobotContainer.indexer.set(Parameters.intake.INTAKE_SPEED))));
    }
}
