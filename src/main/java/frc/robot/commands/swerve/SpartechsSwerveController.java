...
public class SpartechsSwerveController extends CommandBase {
    ...
    public SpartechsSwerveController(PathPlannerTrajectory trajectory, boolean targetLock) {
        ...
        PIDController xPID = new PIDController(Parameters.driveTrain.pid.DEFAULT_LINEAR_MOVE_P, 0, 0);
        PIDController yPID = new PIDController(Parameters.driveTrain.pid.DEFAULT_LINEAR_MOVE_P, 0, 0);
        ProfiledPIDController rotPID = new ProfiledPIDController(Parameters.driveTrain.pid.DEFAULT_ROT_MOVE_P, 0, 0, new Constraints(8, 5));
    ...
    public void execute() {
        ...
        {
            desiredRotation2d = RobotContainer.driveTrain.getEstPose2D().getRotation().rotateBy(Rotation2d.fromDegrees(Vision.getYaw()));
        }
        var targetChassisSpeeds = m_controller.calculate(RobotContainer.driveTrain.getEstPose2D(), desiredState, new Rotation2d());
